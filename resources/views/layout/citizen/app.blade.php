...
<body class="d-flex flex-column antialiased">
    <div class="page">
        {{-- Light top bar — no admin sidebar; citizens just navigate their own area. --}}
        <header class="navbar navbar-expand-md navbar-light d-print-none">
            <div class="container-xl">
                <a href="{{ route('citizen.home') }}" class="navbar-brand fw-bold">
                    <i class="ti ti-ambulance me-1"></i>RescueDasma
                </a>
                <div class="navbar-nav flex-row order-md-last align-items-center">
                    <a href="{{ route('citizen.home') }}" class="nav-link px-2">Home</a>
                    <a href="{{ route('citizen.history') }}" class="nav-link px-2">History</a>
                    <a href="{{ route('citizen.profile') }}" class="nav-link px-2">Profile</a>
                    <a href="{{ route('citizen.medical') }}" class="nav-link px-2">Medical</a>
    ...
    {{-- Feedback modal (SweetAlert alternative) — same API as auth/admin layouts. --}}
    <x-feedback-modal />
    <script>
        (function () {
            ...
            @if (session('error'))
            document.addEventListener('DOMContentLoaded', () =>
                window.feedback.error('Error', @js(session('error')))
            );
            @endif
        })();
    </script>
